...
`lakehouse-admins`, `lakehouse-analysts`, `lakehouse-students`.
...
| Zeppelin | **Shiro + AD (LDAPS)** — OIDC yok (0.12'de pac4j/OIDC realm'i yok) | `activeDirectoryRealm.groupRolesMap` → Shiro rolleri (`runbooks/zeppelin/shiro-ad.ini`) |

Rol eşlemeleri: Superset `glue/values.yaml` → `superset.roleMapping` (`lakehouse-admins: [Admin]`, `analysts: [Alpha]`, `students: [Gamma]`; her girişte `AUTH_ROLES_SYNC_AT_LOGIN`). JupyterHub `platform/values/jupyterhub.yaml` (`allowed_groups` üçü de, `admin_groups` yalnız admins).
...
- **İLK EŞLEŞEN KURAL KAZANIR.** Bu yüzden özel (öğrenci filtre/maske) kuralları genel `SELECT` kuralından ÖNCE yazılır; sırayı bozmak filtreyi sessizce devre dışı bırakır.
...
Canlı davranış (e2e `test/e2e/trino-check/check.py` her koşuda doğrular): `analyst1` → `shop.orders` 3 satır, `remote` maskesiz, `sandbox`'a CTAS **başarılı**; `student1` → aynı tabloda **2 satır** (filtre), `remote='x.x.x.x'` (maske), `sandbox` CTAS **Access Denied**; servis hesabı `e2e` → okur, sandbox'a yazamaz.
...
2. **Trino `rules.json`**: `schemas` → `{"group": "lakehouse-analysts", "schema": "sandbox", "owner": true}`, `tables` → aynı grup için `sandbox` şemasında SELECT/INSERT/DELETE/UPDATE/OWNERSHIP. `lakehouse-students` bu kurallara düşmez → CTAS/INSERT reddedilir.

Analist/öğrenci ayrımı Trino'da, motor sınırı Polaris'te. Bronze/Silver namespace'lerine Trino üzerinden **hiç kimse** yazamaz (oraya yalnız `connect`/`spark` principal'ları yazar).

**Kabul edilen risk:** `platform/values/polaris.yaml` → `features.DROP_WITH_PURGE_ENABLED=true` **sunucu geneldir** (Trino REST katalog `DROP TABLE`'ı `purgeRequested=true` ile gönderir; bayrak olmadan sandbox tablosu silinemez, 403). Sonuç: silme yetkisi olan her principal (connect/spark dâhil) silerken veriyi de purge edebilir. Katalog düzeyine (`polaris.config.drop-with-purge.enabled`) indirmek mümkündür ama `polaris setup apply` katalog `properties`'ini yalnız katalog YARATILIRKEN yazar → var olan kurulumda etkisiz kalır.
