...
## Yürütme ruling'leri (SDD ledger'dan, 2026-09-19 — workspace kaldırılmadan önce kopyalandı)

Kapanış: origin/main == 03e9cae (+ bu commit); cutover 2026-09-19 GitHub rename (kullanıcı koştu) — main = eski v2 tarihi, v1-legacy = eski main; CI main 35435798457 ✅ (49 dk 42 sn), fix dalgası 35438962834; v1 bundle ~/lakehouse-v1-archive/lakehouse-v1-final-2026-09-19.bundle. Kabul demosu v2 uyarlaması kullanıcı kararıyla F6 dışı.

- Ruling: worktree yok, `v2` dalında main checkout (F1–F5 düzeni; lokal kind helm-modu bu checkout'tan kurulur) — yanlışsa maliyet: kirli çalışma ağacı riski (git ile geri alınır).
- Ruling: T2 Step 3 lokal kümede geçici Superset kullanıcısı (`bundle-admin`) yaratıp silmek ürün şablonunu değiştirmez → "runtime mutasyonu yok" kuralını ihlal etmez (üretim adımı, test kümesi) — yanlışsa maliyet: yok (kullanıcı silinir).
- Ruling: T4 Step 3 GitHub dal yeniden adlandırma kullanıcı tarafından açıkça onaylandı (AskUserQuestion "Rename ile") → implementer koşturur; eski dal SİLME komutları kullanıcıya betik olarak verilir (plan/spec §11) — yanlışsa maliyet: rename ters çağrıyla geri alınır.
- Ruling: T3 (docs) T2'den önce koşar — T2 canlı küme hazırlığını (polaris-setup + pg/trino/superset yolları, ~30 dk) bekliyor; T3 kümeden bağımsız; README satırı çakışması ardışıklıkla önlenir — yanlışsa maliyet: yok
- Ruling: monitoring-path.sh kps nesne adlarına sabit (acceptance.sh --mon-ns UWM'de koşamaz) → T2 dispatch'ine ek madde (env ile ezilebilir adlar, Grafana iddiaları atlanabilir) — yanlışsa maliyet: T2 biraz büyür. Review dispatched (sonnet) — package review-432cdf1..f1fe520.diff
- Ruling: #1 çözümü deklaratif — sabit uuid hem `glue/templates/superset.yaml` trino.yaml datasource'unda hem bundle'da (legacy importer uuid'yi honor ediyorsa; canlı doğrulanır), bundle URI/extra ns için `tpl` ile şablonlanır ve ürün datasource'uyla birebir eşit tutulur; honor etmiyorsa runbook import adımı canlı uuid'yi REST'ten okuyup bundle'a yazar — yanlışsa maliyet: import adımı biraz uzar.
- Ruling: #1 çözümü belge+kontrol listesi (SQL müşteri tablolarına uyarlanır; ürün şablonuna tablo listesi values anahtarı EKLENMEZ — YAGNI, bundle canlı üretim gerektirir) — yanlışsa maliyet: operatör bir SQL düzenler.
