...
2. e2e yeşil + runbook'lar tam → **dal yeniden adlandırma** (force-push yerine; geçmiş yazılmaz, geri alınabilir): GitHub REST `POST /repos/{owner}/{repo}/branches/main/rename {new_name: v1-legacy}` (bu çağrı varsayılan dalı otomatik olarak yeni adı `v1-legacy`'ye taşır — GitHub varsayılan dalı **rename edilen dal** ile birlikte taşır, `v2`'ye değil) → `POST .../branches/v2/rename {new_name: main}` → **üçüncü çağrı zorunlu:** `PATCH /repos/{owner}/{repo} {default_branch: main}` (varsayılan dalı açıkça `main`'e — yani şimdi `v2` içeriğini taşıyan dala — geri taşır; açık PR'lar GitHub tarafından otomatik taşınır, varsayılan dal taşınmaz), lokalde `git branch -m main v1-legacy && git branch -m v2 main && git fetch -p && git branch -u origin/main main`. **Silmeden önce** repo dışına `git bundle create ~/lakehouse-v1-archive/lakehouse-v1-final-<tarih>.bundle --all` alınır ve `git bundle verify` ile doğrulanır; sonra `v1-legacy` (GitHub + lokal) ve `archive/*` dalları silinir. Geçmiş-yeniden-yazma/silme komutları bu ortamda gate'e takılırsa adımlar kullanıcıya betik olarak verilir (Foundation Phase 6 emsali).
