...
| `icebergSparkRuntime` | `1.11.0` | `org.apache.iceberg:iceberg-spark-runtime-3.5_2.12` AND `:iceberg-aws-bundle` (version-matched AWS SDK v2 for S3FileIO — single-sourced by the iceberg version, no independent `awsSdkBundle`) |
...
| `icebergKafkaConnect` | `1.11.0` | `org.apache.iceberg:iceberg-kafka-connect-runtime` (Iceberg sink plugin; aligned to `icebergSparkRuntime`) |
