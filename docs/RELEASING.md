...
1. **Builds and pushes every product image to ghcr.io**, tagged both
   ...
   - `ghcr.io/<owner>/lakehouse/spark-py` (`images/spark-py/Dockerfile`)
   ...
   `chart/values.yaml`) is first-party — built from
   `images/spark-py/Dockerfile` (same baked-jars, no-runtime-Ivy pattern as
   `images/connect/Dockerfile`) — and is built + pushed by this workflow's
   `images` matrix like the other four, at this exact release version.
...
`spark-py` (the Spark image for `silver-merge`/`iceberg-maintenance`/
`nginx-ingest`) is built + pushed by this workflow like the three above, and
the coherence bake rewrites `versions.sparkImageTag` too — so it is included
in the four-image coherence contract and a plain default install resolves it
correctly at this release version (no `--set` needed).

`iceberg-tools` is the one published image that is intentionally BYO for chart
purposes (see "Image/chart coherence" below for why it's excluded from the
coherence contract — no chart template references it).
...
**This coherence contract covers exactly four images**:
`connect-lakehouse`, `spark-py`, `console-backend`, `console-frontend` — the
ones this workflow builds and pushes. The bake rewrites
`versions.connectImageTag` / `versions.sparkImageTag` /
`versions.consoleBackendImageTag` / `versions.consoleFrontendImageTag` to the
release version alongside `global.imageRegistry`, so the packaged chart
resolves each — including `spark-py` (rendered by
`chart/templates/05-spark-operator.yaml` and
`chart/templates/14-nginx-ingest.yaml`) — at the exact ghcr.io image this
release published.
...
## Upgrading existing Bronze tables to `__kafka_offset`/`__kafka_partition`

After upgrading to a release that adds the `__kafka_offset`/`__kafka_partition`
columns, an existing Bronze table that receives **no new records** won't gain
those columns via sink schema-evolution, so its next silver-merge tick logs a
column-resolution error and that one table is skipped (the merge job's
aggregate exit code is still `1`) until it either receives a record or is
re-stated (`merge_cdc.py --restate <ns.table>` or `--reset-watermark`). Fresh
installs, and any table that does receive records, are unaffected.
